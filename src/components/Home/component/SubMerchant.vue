<template>
    <div>
        ...
        <div class="SubMerchantHeaderDownloadAll">
            <div class="col">
        ...
        <div class="SubMerchantHeaderTable">
            <table  class="tableHover">
                ...
                <tr class="" style="position: relative">
                    <th class="demoDateModeHover SubMerchantDate" :style="{width: getTextWidth()  +'px'}">
                    ...
                    <th :style="{width: '120px',display: '-webkit-box', borderLeft: '1px solid #d8d8d8', paddingLeft: '10px' }" class="">Action</th>
                </tr>
                </thead>
                <tbody>
        ...
        <Modal v-model="isDownloadAll" width="360" footer-hide>
            <div style="text-align:center">
                ...
                <div class="DownloadAllContent">
                    You are downloading <b>Sub Merchant Summary Reports</b> from {{time.dateStart}} ~ {{time.dateEnd}} <br/>    Proceed?
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            async LoadData () {
                try {
                    this.formData.merchantId =  this.userInfo.merchantId
                    let data = await api.getSummary(this.formData);
            ...
            onDownload (record) {
                window.open('/xmp/v1/report/download/summary?id=' + record.id)
            },
            async LoadUpDown (list) {
                try {
                    let data = await api.downloadTransaction({id: list});
                }
                catch (e) {
                    console.log(e);
                }
            ...
            onDownloadAllUpdata () {
                window.open('/xmp/v1/report/download/all/summary?merchantId='+ this.userInfo.merchantId + '&dateStart=' + formatTime(this.DownloadAllTime[0])+'&dateEnd='+formatTime(this.DownloadAllTime[1]))
                this.onModalClose()
            },
            onModalClose () {
